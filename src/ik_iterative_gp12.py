...
def eval_joint_states(joint_states):
    ...
    for i in range(np.shape(joint_states)[0]):
        T_0 = dh_lib.fk_gp12_arr(joint_states[i])
...
def ik_iterative_gp12(target_pos):
    ...
    while not converged:
        ...
        current_pos = dh_lib.fk_gp12_arr(current_joint_state).dot(base)[0:3, 0]
        k += 1
        # positions.append(current_pos)
        # joint_states.append(current_joint_state)
...
# target_joint_state = ik_iterative_gp12(target_pos)
#
